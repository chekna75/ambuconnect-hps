...
import type { CreateEtablissementDTO, UpdateEtablissementDTO, StatsParams } from '../services/etablissement/types';
...
export const useEtablissement = (id?: string) => {
  ...
  const { data: etablissement, isLoading, error } = useQuery({
    ...
    queryFn: () => etablissementService.getEtablissement(id!),
    enabled: !!id
  });
  ...
  return {
    ...
    isDeactivating: desactiverMutation.isPending
  };
};
...
export const useEtablissementSearch = (query: string) => {
  ...
  return useQuery({
    ...
    queryFn: () => etablissementService.searchEtablissements(query),
